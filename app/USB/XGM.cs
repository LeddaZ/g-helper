...
namespace GHelper.USB
{
    public static class XGM
    {
        ...
        static readonly int[] deviceIds = { 0x1970, 0x1a9a, 0x1C28, 0x1C29, 0x1BC1 };
        ...
        public static void LightMode(AuraMode mode, Color color, Color color2, int speed, int direction)
        {
            Task.Run(() =>
            {
                if (IsConnected())
                {
                    var msg = Aura.AuraMessage(mode, color, color2, speed, direction);
                    msg[0] = XGM_REPORT_ID;
                    Write(msg);
